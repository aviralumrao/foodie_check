...
{ label: "How it works", href: "#how-it-works", id: "navbar-how-it-works-link" },
...
return (
<header className="fixed inset-x-0 top-0 z-50 border-b border-emerald-100/80 bg-white/80 backdrop-blur-xl">
    <nav className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4 lg:px-10" aria-label="Main navigation"
        data-testid="main-navbar">
        <a href="#home" className="group flex items-center gap-3 text-emerald-950" data-testid="navbar-brand-link">
            <span
            ...
            <span className="font-heading text-xl font-black tracking-[-0.04em]" data-testid="navbar-brand-name">
                Foodie Check
            </span>
        </a>

        <div className="hidden items-center gap-8 md:flex" data-testid="navbar-desktop-links">
            {navItems.map((item) => (
            <a key={item.id} href={item.href}
                className="text-sm font-semibold text-emerald-900/65 transition-colors duration-200 hover:text-emerald-700"
                data-testid={item.id}>
                {item.label}
            </a>
            ))}
            ...
            <a href="#scan"
                className="group inline-flex items-center gap-2 rounded-full bg-emerald-700 px-5 py-2.5 text-sm font-bold text-white shadow-lg shadow-emerald-900/15 transition-[transform,box-shadow,background-color] duration-200 hover:-translate-y-0.5 hover:bg-emerald-800 hover:shadow-xl"
    ...
    {menuOpen && (
    <div className="border-t border-emerald-100 bg-white px-6 py-5 md:hidden" data-testid="navbar-mobile-menu">
